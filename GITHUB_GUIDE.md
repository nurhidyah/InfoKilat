...
Salin dan tempel baris demi baris perintah di bawah ini ke terminal:
...
Itu artinya semua file Anda sudah tersimpan di database lokal Git. Jika ini muncul setelah Anda menjalankan perintah `git add .`, Anda bisa langsung melompat ke perintah terakhir yaitu **Langkah 6 (git push)**.

## Mengapa muncul "Repository not found"?
Karena adanya kesalahan ketik pada URL sebelumnya atau masalah autentikasi. Perintah `remote remove` dan `remote add` di atas akan memperbaikinya secara otomatis dengan URL yang sudah saya perbaiki.
